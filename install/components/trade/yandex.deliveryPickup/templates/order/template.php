...
if (empty($arResult["ERRORS"]))
{
	...
	$widgetCode = COption::GetOptionString("yandex.delivery", "basketWidget");
        ...
        if (typeof ydwidget !== "undefined")
        {
            ydwidget.ready(function ()
            {
                ...
                ydwidget.trade_onLoad = function ()
                {
                    ...
                    ydwidget.trade_addressInputs = <?=CUtil::PHPToJSObject($arAddressInputs)?>;
                    ...
                    ydwidget.trade_openWidgetTitles = {
                        "courier": "<?=GetMessage('TRADE_YANDEX_DELIVERY_JS_select_courier')?>",
                        "post": "<?=GetMessage('TRADE_YANDEX_DELIVERY_JS_select_post')?>",
                        "pickup": "<?=GetMessage('TRADE_YANDEX_DELIVERY_JS_select_pickup')?>",
                    };
                ...
                ydwidget.trade_openWidget = function (profile)
                {
					...
					setTimeout(function()
                    {
                        ...
                        if (ydwidget.trade_chosenDeliveryType == "courier" || ydwidget.trade_chosenDeliveryType == "post")
                ...
                {
                    var newTemplateAjax = (typeof(ajaxAns) != 'undefined' && ajaxAns !== null && typeof(ajaxAns.trade_yandex_delivery) == 'object') ? true : false;

                    // выбранная доставка
                    if (ydwidget.trade_oldTemplate)
                    {
                        if (tmpVal = ydwidget.trade_getDataFromAjax("yd_ajaxDeliveryID", "value"))
                            ydwidget.trade_currentdelivery = tmpVal;
                    }
                    else if (newTemplateAjax)
                        ydwidget.trade_currentdelivery = ajaxAns.trade_yandex_delivery.yd_ajaxDeliveryID;
                    ...
                    if (curCheckedProfile == "pickup" && ydwidget.trade_pvzAddressFull)
                        ydwidget.trade_putDataToForm(ydwidget.trade_pvzAddressFull, "yd_pvzAddressValue");
                    else
                        ydwidget.trade_putDataToForm(false, "yd_pvzAddressValue");
                    ...
                    if (ydwidget.trade_oldTemplate)
                    {
                        ...
                        ydwidget.trade_personType = ajaxAns.trade_yandex_delivery.yd_ajaxPersonType;
                    ...
                    if (ydwidget.trade_oldTemplate)
                    {
                        ...
                        ydwidget.trade_currentCity = ajaxAns.trade_yandex_delivery.yd_ajaxLocation;

                    // место, где будет кнопка "выбрать ПВЗ"
                    ydwidget.trade_selectPickupTag = {
                        "pickup": yd$('#yandex_delivery_inject_pickup'),
                        "post": yd$('#yandex_delivery_inject_post'),
                        "courier": yd$('#yandex_delivery_inject_courier')
                    };
                ...
                ydwidget.trade_onDeliveryChange = function (delivery, isAjax)
                {
                    ...
                    ydwidget.trade_setAddressInputs(delivery.address, delivery.type);
                ...
                ydwidget.trade_setAddressInputs = function (addressObj, deliveryType)
                {
                    ...
                    for (var i in ydwidget.trade_addressInputs[ydwidget.trade_personType])
                    {
                        var inputID = ydwidget.trade_addressInputs[ydwidget.trade_personType][i],
                ...
                ydwidget.trade_createAddress = function (delivery)
                {
                    ...
                    if (ydwidget.trade_chosenDeliveryType == "pickup")
                    {
                        ...
                        ydwidget.trade_pvzAddressFull = '<?=GetMessage('TRADE_YANDEX_DELIVERY_JS_PICKUP')?>: ';
                        ydwidget.trade_pvzAddressFull += delivery.full_address + ' | ';
                        ydwidget.trade_pvzAddressFull += delivery.days + ' <?=GetMessage('TRADE_YANDEX_DELIVERY_JS_DAY')?> | ';
                ...
                ydwidget.initCartWidget({
                    ...
                    'order': {
                        ...
                        {
                            return yd$("[name=ORDER_PROP_" + ydwidget.trade_addressInputs[ydwidget.trade_personType]["fname"] + "]").val()
                        },
                        'recipient_last_name': function ()
                        {
                            return yd$("[name=ORDER_PROP_" + ydwidget.trade_addressInputs[ydwidget.trade_personType]["lname"] + "]").val()
                        },
                        'recipient_phone': function ()
                        {
                            return yd$("[name=ORDER_PROP_" + ydwidget.trade_addressInputs[ydwidget.trade_personType]["phone"] + "]").val()
                        },
                        'deliverypoint_street': function ()
                        {
                            return yd$("[name=ORDER_PROP_" + ydwidget.trade_addressInputs[ydwidget.trade_personType]["street"] + "]")
                        },
                        'deliverypoint_house': function ()
                        {
                            return yd$("[name=ORDER_PROP_" + ydwidget.trade_addressInputs[ydwidget.trade_personType]["house"] + "]").val()
                        },
                        'deliverypoint_index': function ()
                        {
                            return yd$("[name=ORDER_PROP_" + ydwidget.trade_addressInputs[ydwidget.trade_personType]["index"] + "]").val()
                        },

                        //объявленная ценность заказа
                        'order_assessed_value': <?=$arResult["TOTAL_PRICE"]?>,
                        //флаг отправки заказа через единый склад.
                        'delivery_to_yd_warehouse': <?=$arParams["TO_DELIVERY_YANDEX_WAREHOUSE"]?>,
